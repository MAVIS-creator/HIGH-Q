<?php
// Handler for Post-UTME registration POST processing.
// This file is intended to be included from public/register.php when
// $registration_type has been determined to be 'postutme'. It expects
// variables like $pdo and sanitized $_POST to be present in scope.
...
try {
    ...
    // Validate JAMB scores lightly (same checks as original)
        ...
        if (in_array(strtolower($ext), $allowed)) {
            ...
            if (move_uploaded_file($u['tmp_name'], $dst)) {
                if (function_exists('app_url')) {
                    ...
                    $fullUrl = $proto . '://' . $host . $scriptDir . '/uploads/passports/' . $fname;
                }
    ...
    try {
        @file_put_contents(__DIR__ . '/../storage/logs/registration_payment_debug.log', date('c') . " CREATE PTU (included handler): posted_reg_type=" . ($posted_reg_type ?: 'NULL') . " detected_reg_type=postutme selectedHasAnyFixed=" . (!empty($selectedHasAnyFixed) ? '1' : '0') . " selectedAllVaries=" . (!empty($selectedAllVaries) ? '1' : '0') . "\n", FILE_APPEND | LOCK_EX);
    } catch (Throwable $_) { }
    ...
    // fall through to render the registration page with errors
}
