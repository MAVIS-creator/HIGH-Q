...
try {
    if ($method === 'GET') {
        ...
        $chk = $pdo->prepare('SELECT 1 FROM comment_likes WHERE comment_id = ? AND (session_id = ? OR ip = ?) LIMIT 1'); $chk->execute([$commentId, $sessionId, $ip]);
    ...
    if ($method === 'POST') {
        ...
        $ins->execute([$commentId, $sessionId, $ip]);
