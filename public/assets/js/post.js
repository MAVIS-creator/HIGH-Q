...
(function () {
  'use strict';
  const HQ_BASE = (window.HQ_BASE_URL || '').replace(/\/$/, '');
  ...
  function fetchComments() {
    fetch(HQ_BASE + '/public/api/comments.php?post_id=' + encodeURIComponent(POST_ID))
      .then(r => r.json())
      .then(data => {
  ...
  const delBtn = c.can_delete ? ' <button class="delete-btn" data-id="' + (c.id||'') + '"><i class="bx bx-trash"></i> Delete</button>' : '';
    ...
    likeBtnC.addEventListener('click', function () {
      ...
      fetch(HQ_BASE + '/public/api/comment_like.php', { method: 'POST', body: new URLSearchParams({ comment_id: cid }) })
        .then(r => r.json())
        .then(j => {
  ...
  function doDelete(cid) {
    ...
    fetch(HQ_BASE + '/public/api/comments.php', { method: 'POST', body: fd })
      .then(r => r.json())
  ...
  if (likeBtn && POST_ID) {
    ...
    fetch(HQ_BASE + '/public/api/like_post.php?post_id=' + encodeURIComponent(POST_ID))
      .then(r => r.json())
      .then(j => {
    ...
    likeBtn.addEventListener('click', function () {
      ...
      fetch(HQ_BASE + '/public/api/like_post.php', { method: 'POST', body: new URLSearchParams({ post_id: POST_ID }) })
        .then(r => r.json())
        .then(j => {
          if (j && typeof j.likes !== 'undefined') likesCountEl.textContent = j.likes;
