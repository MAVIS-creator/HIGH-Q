        ...
        (function(){
          ...
          try {
            (function(){
              ...
              var apiBase = (window.HQ_APP_BASE || window.HQ_BASE || '').replace(/\/$/, '');
              fetch(apiBase + '/api/activate_payment.php', { method: 'POST', body: fd, credentials: 'same-origin', headers: { 'X-Requested-With':'XMLHttpRequest' } })
          ...
          function check(){
            ...
            var apiBase = (window.HQ_APP_BASE || window.HQ_BASE || '').replace(/\/$/, '');
            var url = apiBase + '/api/payment_status.php?ref=' + encodeURIComponent(ref) + '&t=' + Date.now();
