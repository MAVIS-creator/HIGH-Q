...
try {
  ...
  $i = 0;
  foreach ($nodes as $n) { $txt = trim($n->textContent); $id = $n->getAttribute('id'); if (!$id) { $id = preg_replace('/[^a-z0-9]+/i','-',strtolower($txt)); $id = trim($id,'-') . '-' . (++$i); $n->setAttribute('id',$id); } $toc[]=['id'=>$id,'text'=>$txt,'level'=>(strtolower($n->nodeName)==='h3'?3:2)]; }
  $body = $doc->getElementsByTagName('body')->item(0); $div = $body ? $body->getElementsByTagName('div')->item(0) : null;
  ...
  <div class="post-grid">
    <article class="post-article">
      ...
      <div class="post-content" style="margin-top:12px;">
        <?php if (!empty($post['excerpt'])): ?><div class="post-excerpt"><?= nl2br(htmlspecialchars($post['excerpt'])) ?></div><?php endif; ?>
        <?= $rendered ?>
      </div>
    </article>

    <aside class="post-sidebar">
      <?php $likesCount = 0; try { $ls = $pdo->prepare('SELECT COUNT(*) FROM post_likes WHERE post_id = ?'); $ls->execute([(int)$post['id']]); $likesCount = (int)$ls->fetchColumn(); } catch (Throwable $e) { if (isset($post['likes'])) $likesCount = (int)$post['likes']; } $commentsCount = max(0, count($allComments)); ?>
      <div class="post-actions">
        <div class="post-stats">
          <button id="likeBtn" class="icon-btn" title="Like" aria-label="Like">
            <!-- heart SVG -->
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 21s-7.5-4.35-10-7.05C-0.1 10.2 2.2 6 6 6c2.1 0 3.5 1.5 4 2.2C10.5 7.5 11.9 6 14 6c3.8 0 6.1 4.2 4 7.95C19.5 16.65 12 21 12 21z" fill="#E23B3B"/></svg>
            <span class="count" id="likesCount"><?= $likesCount ?></span>
          </button>

          <button class="icon-btn" title="Comments" aria-label="Comments">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 15a2 2 0 0 1-2 2H8l-5 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" fill="#6B7280"/></svg>
            <span class="count"><?= $commentsCount ?></span>
          </button>

          <button id="shareBtn" class="icon-btn" title="Share" aria-label="Share">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 12v7a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-7" stroke="#111" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/><path d="M16 6l-4-4-4 4" stroke="#111" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/><path d="M12 2v13" stroke="#111" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </button>
        </div>
      </div>
  ...
  <section id="commentsSection" class="comments-section">
    ...
    <div class="comment-form-wrap">
      ...
      <form id="commentForm" class="community-comment-form">
        ...
        <div class="honeypot" style="display:none"><input type="text" name="hp_name" autocomplete="off" tabindex="-1"></div>
...
<script>
const POST_ID = <?= json_encode((int)$post['id']) ?>;
...
<div class="container" style="max-width:1100px;margin:24px auto;padding:0 12px;">
  <div class="post-grid">
    ...
    // build TOC and render content safely
    $rendered = '';
    $toc = [];
    try {
      libxml_use_internal_errors(true);
      $doc = new DOMDocument(); $doc->loadHTML('<div>' . $post['content'] . '</div>');
      $xpath = new DOMXPath($doc); $nodes = $xpath->query('//h2|//h3'); $i=0;
      foreach ($nodes as $n) { $txt = trim($n->textContent); $id = $n->getAttribute('id'); if (!$id) { $id = preg_replace('/[^a-z0-9]+/i','-',strtolower($txt)); $id = trim($id,'-') . '-' . (++$i); $n->setAttribute('id',$id); } $toc[]=['id'=>$id,'text'=>$txt,'level'=> (strtolower($n->nodeName)==='h3'?3:2)]; }
      $body = $doc->getElementsByTagName('body')->item(0); $div = $body ? $body->getElementsByTagName('div')->item(0) : null;
      if ($div) { foreach ($div->childNodes as $cn) $rendered .= $doc->saveHTML($cn); }
      $allowed = '<h1><h2><h3><h4><p><ul><ol><li><strong><em><a><img><br><blockquote><pre><code>';
