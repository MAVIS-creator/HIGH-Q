...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // Temporarily disabled reCAPTCHA verification (uncomment & implement if needed)
                ...
                <div class="card-body p-4 p-lg-5">
                    ...
                    <?php if (!empty($recfg['site_key'])): ?>
                        ...
                        (function(){
                            ...
                            var w = document.createElement('div'); w.className='g-recaptcha';
                            w.setAttribute('data-sitekey','<?= htmlspecialchars($recfg['site_key']) ?>');
                            w.style.marginTop='12px'; f.insertBefore(w, f.querySelector('button'));
                        })();
                        </script>
                    <?php endif; ?>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(openSchedule && modal){
        openSchedule.addEventListener('click', function(){ modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); });
        openSchedule.addEventListener('keypress', function(e){ if(e.key==='Enter') openSchedule.click(); });
    }
    if(cancel){
        cancel.addEventListener('click', function(){ modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); });
    }
    if(confirm){
        confirm.addEventListener('click', function(){
            var date=document.getElementById('visit_date').value;
            var time=document.getElementById('visit_time').value;
            if(!date){ if (typeof Swal !== 'undefined') Swal.fire('Oops','Please pick a date','warning'); else alert('Please pick a date'); return; }
            var msg = 'Thanks — your visit has been requested for ' + date + ' at ' + time + '. Our team will contact you to confirm.';
            if (typeof Swal !== 'undefined') Swal.fire('Request Sent', msg, 'success'); else alert(msg);
    ...
    if(floatBtn){
        floatBtn.addEventListener('click', function(e){ e.preventDefault(); location.href = 'contact.php#livechat'; });
    }
