...
<!-- Fallback / robust handlers for Read more and Schedule modal (ensures functionality even if other scripts fail) -->
<script>
if (!window.hq_contact_enhanced) {
	window.hq_contact_enhanced = true;
	document.addEventListener('DOMContentLoaded', function(){
		try {
			/* Read more / Show less for FAQ paragraphs: works on all screen sizes */
			(function(){
				var clampLines = 4;
				var cards = document.querySelectorAll('.faq-card p');
				cards.forEach(function(p, idx){
					// ensure we only add a button once
					...
					if (p.scrollHeight > maxH + 2) {
						...
						btn.addEventListener('click', function(){
							...
							if (!expanded) {
								p.style.maxHeight = '';
								p.classList.add('faq-clamped--expanded');
								btn.setAttribute('aria-expanded','true');
								btn.textContent = 'Show less';
								// ensure visible
								setTimeout(function(){ p.scrollIntoView({behavior:'smooth', block:'nearest'}); }, 60);
							} else {
								p.style.maxHeight = maxH + 'px';
								p.classList.remove('faq-clamped--expanded');
								btn.setAttribute('aria-expanded','false');
								btn.textContent = 'Read more';
							}
						});
			...
			/* Schedule modal: robust open/close handlers */
			(function(){
				var openSchedule = document.getElementById('openSchedule');
				var modal = document.getElementById('modalBackdrop');
				var cancel = document.getElementById('cancelSchedule');
				...
				if (confirm) confirm.addEventListener('click', function(e){ /* reuse existing confirm behavior if present */ });
		...
		} catch (err) { console && console.error && console.error('contact enhancements failed', err); }
	});
}
</script>
