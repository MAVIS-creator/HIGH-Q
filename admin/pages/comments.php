...
if (!empty($_GET['ajax'])) {
  foreach($comments as $c){
    ...
    foreach($replies as $rep) {
      ...
      if ($rep['user_id']) {
        echo "<td><i class='bx bxs-shield-check' style=\"color:#10b981;\"></i> ".htmlspecialchars($displayName)."</td>";
      } else {
        echo "<td>".htmlspecialchars($displayName)."</td>";
      }
      echo "<td>".htmlspecialchars(mb_strimwidth($rep['content'],0,180,'...'))."</td>";
      echo "<td><span class='status-badge status-reply'>Reply</span></td>";
      echo "<td><small>".htmlspecialchars($rep['created_at'])."</small></td>";
      echo "<td>&mdash;</td>";
      echo "</tr>";
    }
  }
